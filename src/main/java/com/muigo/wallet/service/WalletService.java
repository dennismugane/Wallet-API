...
public class WalletService {
	...
	private WalletResponse doDeposit(DepositRequest request) {
		...
		Wallet saved = walletRepository.save(wallet);
		recordTransaction(Transaction.builder().walletId(saved.getId()).type(TransactionType.DEPOSIT)
				.amount(request.getAmount()).balanceAfter(saved.getBalance()).description(request.getDescription()).build());
		recordSuccess("deposit");
		transactionAmountSummary.record(request.getAmount().doubleValue());
		log.info("Deposited {} to wallet {}. New balance: {}", request.getAmount(), saved.getId(), saved.getBalance());
		return toResponse(saved);
	}
	...
	private WalletResponse doWithdraw(WithdrawRequest request) {
		...
		Wallet saved = walletRepository.save(wallet);
		recordTransaction(Transaction.builder().walletId(saved.getId()).type(TransactionType.WITHDRAWAL)
				.amount(request.getAmount()).balanceAfter(saved.getBalance()).description(request.getDescription()).build());
		recordSuccess("withdrawal");
		transactionAmountSummary.record(request.getAmount().doubleValue());
		log.info("Withdrew {} from wallet {}. New balance: {}", request.getAmount(), saved.getId(), saved.getBalance());
		return toResponse(saved);
	}
	...
	private TransferResponse doTransfer(TransferRequest request) {
		...
		Wallet first = walletRepository.findByIdWithLock(firstId).orElseThrow(() -> {
			recordError("wallet_not_found", "transfer");
			return new WalletNotFoundException(firstId);
		});
		Wallet second = walletRepository.findByIdWithLock(secondId).orElseThrow(() -> {
			recordError("wallet_not_found", "transfer");
		...
		recordTransaction(Transaction.builder().walletId(from.getId()).referenceId(referenceId)
				.type(TransactionType.TRANSFER_DEBIT).amount(request.getAmount()).balanceAfter(from.getBalance())
				.description(request.getDescription()).build());
		recordTransaction(Transaction.builder().walletId(to.getId()).referenceId(referenceId)
				.type(TransactionType.TRANSFER_CREDIT).amount(request.getAmount()).balanceAfter(to.getBalance())
				.description(request.getDescription()).build());
		recordSuccess("transfer");
		transactionAmountSummary.record(request.getAmount().doubleValue());
		log.info("Transferred {} from wallet {} to wallet {}. Ref: {}", request.getAmount(), from.getId(), to.getId(), referenceId);
		return TransferResponse.builder().referenceId(referenceId).amount(request.getAmount())
				.fromWallet(toResponse(from)).toWallet(toResponse(to)).build();
	}

	@Transactional(readOnly = true)
	public WalletResponse getWallet(String walletId) {
		return walletRepository.findById(walletId).map(this::toResponse)
	...
	private void recordSuccess(String type) {
		Counter.builder("wallet.transactions.total").tag("type", type).tag("status", "success")
				.register(meterRegistry).increment();
	}

	private void recordInsufficientFunds(String type) {
		Counter.builder("wallet.transactions.total").tag("type", type).tag("status", "insufficient_funds")
				.register(meterRegistry).increment();
		Counter.builder("wallet.errors.total").tag("type", "insufficient_funds").tag("operation", type)
				.register(meterRegistry).increment();
	}

	private void recordError(String errorType, String operation) {
		Counter.builder("wallet.errors.total").tag("type", errorType).tag("operation", operation)
				.register(meterRegistry).increment();
	}
	...
	private WalletResponse toResponse(Wallet wallet) {
		return WalletResponse.builder().id(wallet.getId()).ownerName(wallet.getOwnerName())
				.balance(wallet.getBalance()).createdAt(wallet.getCreatedAt()).updatedAt(wallet.getUpdatedAt()).build();
	}

	private TransactionResponse toTransactionResponse(Transaction transaction) {
		return TransactionResponse.builder().id(transaction.getId()).walletId(transaction.getWalletId())
				.referenceId(transaction.getReferenceId()).type(transaction.getType()).amount(transaction.getAmount())
				.balanceAfter(transaction.getBalanceAfter()).description(transaction.getDescription())
